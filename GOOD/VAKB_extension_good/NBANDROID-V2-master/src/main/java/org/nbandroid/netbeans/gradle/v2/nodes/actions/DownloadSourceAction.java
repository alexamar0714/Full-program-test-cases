...
@ActionID(
        ...
        id = "org.nbandroid.netbeans.gradle.v2.nodes.actions.DownloadSourceAction"
)
@ActionRegistration(
        displayName = "", lazy = false
)
public class DownloadSourceAction extends NodeAction {
    ...
    protected boolean enable(Node[] activatedNodes) {
        for (Node n : activatedNodes) {
            ...
            if (data.isSrcLocal() || data.isLocal() || !data.isDownloadable()) {
                return false;
            }
        }
        return true;
