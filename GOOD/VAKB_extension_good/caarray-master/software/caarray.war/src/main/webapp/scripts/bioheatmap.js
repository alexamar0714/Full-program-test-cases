...
org.systemsbiology.visualization.BioHeatMap = Class.create({
    ...
    initialize: function(container) {
        ...
        // private variables
        //  - defaults
        this._font = "Courier New, monospace";
    ...
    draw: function(data, options) {
        ...
        if (ctx) {
            ...
            this.canvas.onmouseenter = this._getMouseXY(); // mouse event handler
            this.canvas.onmousemove = this._getMouseXY(); // mouse event handler
            this.canvas.onmouseout = function(e) { console.log("mouseout!!!!"); google.visualization.events.trigger(this, 'mouseout', {originalEvent : e}) };    	    	
    ...
    _drawRowLabels: function(data, rowNameIndex) {
        if (this._useRowNames) {
            for (var row = 0; row < this._numRows; row++) {
                ...
                this.ctx.font = "" + this._fontHeight + "px " + this._font;
            	var w = this.ctx.measureText(rowName).width;
            	this.ctx.textAlign = 'end';
                this.ctx.strokeText(rowName, topLeftPoint.x, topLeftPoint.y);
            }
        }
    },
...
//        }
    ...
    _calcMaxRowLabelWidth: function(data) {
        ...
        for (var row = 0; row < data.getNumberOfRows(); row++) {
            ...
            var rowNameWidth = this.ctx.measureText(rowName).width;
            console.log("width of " + rowName + " is " + rowNameWidth);
            if (rowNameWidth > this._rowLabelWidth) {
                this._rowLabelWidth = rowNameWidth;
            }
        }
        this._rowLabelWidth = 150; //Math.ceil(this._rowLabelWidth);
    },
    ...
    _calcMaxColumnLabelHeight: function(data) {
        ...
        for (var col = colStartIndex; col < data.getNumberOfColumns(); col++) {
            var colName = data.getColumnLabel(col);
            var colNameWidth = this.ctx.measureText(colName).width;
            console.log("height of " + colName + " is " + colNameWidth);
            if (colNameWidth > this._columnLabelHeight) {
                this._columnLabelHeight = colNameWidth;
            }
        }
        this._columnLabelHeight = 150; //Math.ceil(this._columnLabelHeight);
    },
    ...
    _onMouseEvent: function(point, event) {
    	console.log("Mouse event of type " + event.type);
        ...
        if (cell) {
            ...
            if (col >= 0 && row == -1) {
                ...
                if (!this._useRowNames || col != 0) {
                    ...
                    this._setSelectionWithEvent([{column:col}], event);
                }
            } else if (col == 0 && row >= 0 && this._useRowNames) {
                this._log("row label selected, row:" + row);
                this._setSelectionWithEvent([{row:row}], event);
            } else if (col >= 0 && row >= 0) {
                this._log("Selcted Cell - r:" + cell.row + ", c:" + cell.col);
                this._setSelectionWithEvent([{row:row, column:col}], event);
            } else {
                this._log("No cell actually selected");
                this.setSelection([]);
            }
        } else {
            this._log("No cell actually selected");
    ...
    _setSelectionWithEvent: function(selections, event) {
        this.setSelection(selections);
        var sentEventType = event.type == 'click' ? 'select' : 'hover';
        google.visualization.events.trigger(this, sentEventType, {originalEvent : event});    	    	
    },
    ...
    _getMouseXY: function() {
        ...
        return function(e) {
            ...
            if (t) {
                ...
                var point = {x:x,y:y};
                bioHeatMap._onMouseEvent(point, e)
                return point;
            }
...
//        this._log("Value="+dataValue+", colorString="+colorString);
    ...
    _setupColorRange: function() {
        ...
        var centerColor = {r:255, g:255, b:255, a:1};
        var colorStep;
