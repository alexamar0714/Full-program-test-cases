...
public class VisualizationTimeline extends Visualization {
	...
	public String generate(MethodResult methodResult, String[] visualizationParameters) {
		...
		try {
			...
			resultHTML.append(
					"google.load('visualization', '1', {packages: ['annotatedtimeline'], callback: qv_drawChart});\n");
			resultHTML.append("}\n");
			...
			while (iterator.hasNext()) {
				...
				for (int i = 0; i < columnCount; i++) {
					...
					case Types.TIME:
					case Types.TIMESTAMP:
						resultHTML.append(" new Date(").append(((Timestamp) currentRow[i]).getTime()).append(")");
						break;
					case Types.BOOLEAN:
					case Types.BIGINT:
					case Types.DECIMAL:
			...
			resultHTML.append(
					"var chart = new google.visualization.AnnotatedTimeLine(document.getElementById('" + randomNodeId)
					.append("'));\n");
