...
/**
 * VisualizationRadarChart.java <br>
...
public class VisualizationRadarChart extends Visualization {
	...
	public String generate(MethodResult methodResult, String[] visualizationParameters) {
		...
		try {
			...
			resultHTML
					.append("google.load('visualization', '1', {packages: ['imagechart'], callback: qv_drawChart});\n");
			resultHTML.append("}\n");
			...
			resultHTML.append(
					"  optionsScaleMax = optionsScaleMax < data.getColumnRange(i).max ? data.getColumnRange(i).max : optionsScaleMax;\n");
			resultHTML.append("}\n");
			...
			resultHTML
					.append("var chart = new google.visualization.ImageChart(document.getElementById('" + randomNodeId)
					.append("'));\n");
