    ...
    <div class="wrapper">
      ...
      <div class="content-wrapper">
        ...
        <section class="content-header">
        	...
        	<div class="row">        	
				<div class="col-md-4" id="mostrarOcultarPeriodo">
				</div>
				<div class="col-md-6" id="mostrarOcultarEtiqueta">
				</div>						
			</div>
...
/* 					function getColorDesemp(val){
					...
					var etiquetaActual = usuarioEtiqueta[0].etiqueta_id;
					var depto_id =  null;
					var dist_id = null;
...
/* 								for (var i = 0; i< instituciones.length;i++){
								...
								var a=renderTableroLineaAccion(todasInstituciones,e.target.feature.properties.dpto,e.target.feature.properties.distrito,periodoActual,etiquetaActual);
								...
								var todasInstituciones=getInstitucionesSeleccionadas();
								var a=renderTableroLineaAccion(todasInstituciones,e.target.feature.properties.dpto,null,periodoActual,etiquetaActual);
								dist_id = null;
								depto_id = e.target.feature.properties.dpto;
								getPeriodo();
								getEtiqueta();
							...
							var etiquetaSeleccionada = $("#etiquetaSeleccion option:selected").val();
	...
	if(todasLasLineasAccion != ""){
		...
		{ 
		  	for(var n=0; n<lineasProgramadas.length;n++)
			{
				...
				{
					if(usuarioEtiqueta[l].etiqueta_id == etiqueta)
					{
						for(var t = 0; t < insLineaAccionHasEtiqueta.length; t++)
						{
							if(insLineaAccionHasEtiqueta[t].ins_linea_accion_id == lineasProgramadas[n].insLineaAccionId && insLineaAccionHasEtiqueta[t].etiqueta_id == etiqueta)
							{
...
function redibujarGeo(){
   	periodoSeleccionado = $("#periodoSeleccion option:selected").val();
   	etiquetaSeleccionada = $("#etiquetaSeleccion option:selected").val();
	...
	var institucion_idConcat=getInstitucionesSeleccionadas();
		...
		if((depto_id !==null) && (dist_id !==null)){
			...
			var institucion_idConcat=getInstitucionesSeleccionadas();
			var a = renderTableroLineaAccion(institucion_idConcat,depto_id,dist_id,periodoSeleccionado,etiquetaSeleccionada);
			$("#cuerpoTableroLineaAccion").html("");
			$("#cuerpoTableroLineaAccion").html(a);
		}else{
			if(depto_id !==null && dist_id == null){
				...
				var todasInstituciones=getInstitucionesSeleccionadas();
				var a = renderTableroLineaAccion(todasInstituciones,depto_id,null,periodoSeleccionado,etiquetaSeleccionada);
				$("#cuerpoTableroLineaAccion").html("");
				$("#cuerpoTableroLineaAccion").html(a);
			}else{
				...
				for (var i = 0; i< instituciones.length;i++){
					...
					var a = renderTableroLineaAccion(institucion_idConcat,null,null,periodoSeleccionado,etiquetaSeleccionada);
					$("#cuerpoTableroLineaAccion").html("");
					$("#cuerpoTableroLineaAccion").html(a);
...
function getPeriodo(periodo){
	...
	var periodoCuerpo = '<div class="col-sm-12">'+
							'<label for="periodoSeleccion" style="padding-top:6px;">Periodo: </label>'+
							'<select id="periodoSeleccion" class="form-control" style="!important; display: inline-block;">'+optionPeriodo+'</select>'+
						'</div>';
...
function getEtiqueta(etiqueta){

	usuarioEtiqueta = $.ajax({
		url:'/tablero/ajaxSelects2?action=getUsuarioEtiqueta',
	  	type:'get',
	  	dataType:'json',
	  	async:false       
	...
	//rellenado las etiquetas en opciónEtiqueta.
	var optionEtiqueta;		
	if(usuarioEtiqueta.length > 0){
		for(var d = 0; d<usuarioEtiqueta.length; d++){
			for(e = 0;e<etiqueta.length; e++){	
	...
	var etiquetaCuerpo = '<div class="col-sm-12">'+
							'<label for="etiquetaSeleccion" style="padding-top:6px;">Etiqueta: </label>'+
							'<select id="etiquetaSeleccion" class="form-control" style="!important; display: inline-block;">'+optionEtiqueta+'</select>'+
						'</div>';
						
	$('#mostrarOcultarEtiqueta').html(etiquetaCuerpo);
	...
	if (institucion_idConcat==""){
		...
		if(institucion_idConcat != "" && (depto_id !==null) && (dist_id !==null)){
			...
			if(institucion_idConcat != "" && (depto_id !==null)){
				var a = renderTableroLineaAccion(institucion_idConcat,depto_id, null, periodoSeleccionado,etiquetaSeleccionada);
				$("#cuerpoTableroLineaAccion").html("");
				$("#cuerpoTableroLineaAccion").html(a);
			}else{
				if(institucion_idConcat != ""){
...
$(document).ready(function(){
	...
	var todasInstituciones=getInstitucionesSeleccionadas();
	var a=renderTableroLineaAccion(todasInstituciones,null,null,periodoActual,etiquetaActual);
	getPeriodo();
	getEtiqueta();
	$("#cuerpoTableroLineaAccion").html('');
	$("#cuerpoTableroLineaAccion").html(a);
