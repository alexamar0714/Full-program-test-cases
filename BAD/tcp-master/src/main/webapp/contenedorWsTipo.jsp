...
<script>
	...
	$(document).ready(function(){
		...
		var wsTipo = $.ajax({
			url:'/tablero/ajaxSelects2?action=getWsTipo',
		  	type:'get',
		  	dataType:'json',
		  	async:false       
		}).responseText;		
		...
		$("body").on("click", ".registrosWsTipo",function(event){
			...
			$("#borradoWsTipo").remove();
			$("#guardarWsTipo").remove();
			$('#wsTipo').modal('show');
			$("#wsTipo").find(".form-horizontal").append('<div class="form-group" id="borradoWsTipo"><div class="col-lg-offset-2 col-lg-10"><button type="submit" class="btn btn-success" id="actualizarWsTipo">Actualizar</button></div></div>');
			$("#idWsTipo").val(wsTipo[codigoRegistro].id);
			$("#nombreWsTipo").val(wsTipo[codigoRegistro].nombre);
			$("#descripcionWsTipo").val(wsTipo[codigoRegistro].descripcion);
		});
