...
<script>
	...
	$(document).ready(function(){
		...
		$("body").on("click", ".registro",function(event){
			...
			var registros = $.ajax({
		    	url:'/tablero/ajaxSelects?db=base&action=getFactHitos2015&institucion_id='+institucion_id+'&linea_accion_id='+linea_accion_id+'&departamento='+idDepartamento,
		      	type:'get',
		      	dataType:'json',
		      	crossDomain:true,
		      	async:false       
			...
			lineaAccionAcumuladoMesDepto = $.ajax({
		    	url:'/tablero/ajaxSelects?db=base&action=getLineaAccionAcumuladoMesDepto&institucion_id='+institucion_id+'&linea_accion_id='+linea_accion_id+'&departamento='+idDepartamento,
		      	type:'get',
		      	dataType:'json',
		      	async:false       
		    }).responseText;
		...
		var lineaAccionDepartamento = $.ajax({
	    	url:'/tablero/ajaxSelects?db=base&action=getLineasAccionDepartamento',
	      	type:'get',
	      	dataType:'json',
	      	crossDomain:true,
	      	async:false       
						...
						if(lineaAccionDepartamento[j].anho<="2014"){
							...
							//	var porcentajeAnho = (anho2.suma_programada_hoy*100)/anho2.suma_programada_anho;
								...
								*/
								...
								if (porcentajeAnho<90){ porHejeClassRow="none";porHejeClass="";}
								if (porcentajeAnho>=90){ porHejeClassRow="none";
		...
		function renderEntidades(e){
			...
			for (var i = 0; i< entidades.length;i++){
				...
				if (!isNaN(porcentajeAnhoAcumuladoTotal) && !isNaN(porcentajeHoyEjeAcumuladoTotal)){
					...
					if(porcentajeHoyEjeAcumuladoTotal.toFixed() >= 90)
					{
						$("#tablaInstituciones").append('<tr><td>'+entidades[i].institucion+'</td><td><div class="progress progress-xs"> <div class="progress-bar bg-'+'gray'+'-active color-palette" style="width: '+porcentajeHoyEjeAcumuladoTotal+'%"><p class="text-left" style="color:black;padding-left:10px"><small>'+porcentajeHoyEjeAcumuladoTotal.toFixed(2)+'%</small></p></div></div></td></tr>');
					}
		...
		function renderEntidades2(e){
			var array=[];
			$("#tablaInstituciones2").html("");
			if (typeof e != 'undefined'){
				array=lineaAccionDepartamento;
			}else{
				array=elPais;
			...
			for (var i = 0; i< entidades.length;i++){
				...
				if (!isNaN(porcentajeAnhoAcumuladoTotal) && !isNaN(porcentajeHoyEjeAcumuladoTotal)){
					...
					if(porcentajeHoyEjeAcumuladoTotal.toFixed() < 90)
					{
						$("#tablaInstituciones2").append('<tr><td>'+entidades[i].institucion+'</td></tr>');
					}
					porcentajeAnhoAcumuladoTotal=0;
					porcentajeAnhoAcumuladoDeptoTotal=0;
	          ...
	          <div class="box" height="1000px">
	            <div class="box-header with-border" height="1000px">
	              <h2 class="box-title text-center" id="tituloTipoPrograma">
	                Instituciones con Programación mayor o igual a 90% 	
	              </h2>
